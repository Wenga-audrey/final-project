...
// STUDY GROUP ROUTES - Moved here to avoid route conflicts
...
router.get("/study-groups/:groupId", async (req, res, next) => {
  try {
...
// Join a study group
router.post("/study-groups/:groupId/join", authenticate, async (req, res, next) => {
  try {
    const { groupId } = req.params;
    const userId = req.user.id;
...
// Leave a study group
router.post("/study-groups/:groupId/leave", authenticate, async (req, res, next) => {
  try {
    const { groupId } = req.params;
    const userId = req.user.id;
...
router.get("/study-groups/:groupId/messages", authenticate, async (req, res, next) => {
  try {
    const { groupId } = req.params;
    const userId = req.user.id;
    ...
    if (!membership) {
      return res.status(403).json({ error: "You must be a member of this study group to view messages" });
    }
...
router.post("/study-groups/:groupId/messages", authenticate, async (req, res, next) => {
  try {
    const { groupId } = req.params;
    ...
    if (!membership) {
      return res.status(403).json({ error: "You must be a member of this study group to send messages" });
    }
